...
var KickstarterGraph = (function () {
  function KickstarterGraph(opts) {
    ...
    this.config = opts;
    this.database = new _firebase2['default'](opts.firebase);
    this.data = [];
  ...
  _createClass(KickstarterGraph, [{
    ...
    value: function accumulativeTimeSeries() {
      ...
      var size = _d32['default'].select(el).node().getBoundingClientRect();
      var m = { top: 20, right: 20, bottom: 30, left: 50 };
      var w = size.width - m.left - m.right;
      var h = size.height - m.top - m.bottom;
      var tooltipDateFormat = _d32['default'].time.format('%d %b %H:%M%p');
      var data = [];
      var pledgedArr = [];
      var dateArr = [];
      var missingData = [];
      ...
      if (this.data.overview.launched_at < _d32['default'].min(dateArr)) {
      ...
      x = _d32['default'].time.scale().range([0, w]);

      y = _d32['default'].scale.linear().range([h, 0]);

      xAxis = _d32['default'].svg.axis().scale(x).orient('bottom').ticks(_d32['default'].time.days, 2);

      yAxis = _d32['default'].svg.axis().scale(y).orient('left');

      line = _d32['default'].svg.line().x(function (d) {
        return x(d.date);
      }).y(function (d) {
        return y(d.pledged);
      });

      svg = _d32['default'].select(this.config.el).append('svg').attr('width', w + m.left + m.right).attr('height', h + m.top + m.bottom).append('g').attr('transform', 'translate(' + m.left + ', ' + m.top + ')');

      if (this.missingData) {
        y.domain([0, _d32['default'].max(pledgedArr)]);
        x.domain([missingData[0].date, new Date(dateArr[dateArr.length - 1] * 1000)]);
      } else {
        y.domain(_d32['default'].extent(data, function (d) {
          return d.pledged;
        }));
        x.domain(_d32['default'].extent(data, function (d) {
          return d.date;
        }));
      }
      ...
      svg.selectAll('dot').data(data).enter().append('circle').attr('class', 'dot').attr('r', 0).attr('cy', function (d) {
        ...
        _d32['default'].select(this).transition().duration(200).style('r', 5);
        ...
        _d32['default'].select(this).transition().duration(200).style('r', 0);
...
new KickstarterGraph({
  el: '.graph',
  firebase: (0, _jquery2['default'])('body').attr('data-firebase'),
  view: 'accumulativeTimeSeries'
});

},{"d3":1,"firebase":2,"jquery":3}]},{},[4])
